...
const INSTRUCTIONS_BLOCK_START = "<!-- skillsync:instructions:start -->";
const INSTRUCTIONS_BLOCK_END = "<!-- skillsync:instructions:end -->";
...
function formatPathForDoc(targetRoot, filePath) {
  const relativePath = path.relative(targetRoot, filePath);
  return relativePath && !relativePath.startsWith("..") ? toPosixPath(relativePath) : filePath;
}
...
function resolveRoots(targetRoot, options = {}) {
  return {
    targetRoot,
    sourceRoot: options.sourceRoot ? path.resolve(options.sourceRoot) : targetRoot,
  };
}

async function loadClaudePluginMetadata(sourceRoot) {
  const pluginPath = path.join(sourceRoot, ".claude-plugin", "plugin.json");
  const raw = await readIfExists(pluginPath);
  if (!raw) {
    return null;
  }
  ...
  try {
    ...
    throw new Error(`Invalid Claude plugin metadata at ${pluginPath}: ${error.message}`);
  }
}
...
async function discoverArtifacts(sourceRoot, config) {
  const artifacts = [];

  const skillsRoot = path.join(sourceRoot, config.sources.skills);
  ...
  const agentsRoot = path.join(sourceRoot, config.sources.agents);
  for (const filePath of await walkFiles(agentsRoot)) {
    const baseName = path.basename(filePath);
    if (baseName !== "agent.md" && baseName !== "AGENT.md") {
      continue;
    }
    const packageRoot = path.dirname(filePath);
  ...
  const commandsRoot = path.join(sourceRoot, config.sources.commands);
...
async function normalizeArtifact(artifact) {
  ...
  if (artifact.kind === "command") {
    metadata.namespace = artifact.entry.data.namespace || "";
    metadata.commandName = metadata.namespace
      ? `${metadata.namespace}:${artifact.name}`
      : artifact.name;
    metadata.slash = artifact.entry.data.slash || `/${metadata.commandName}`;
  }
...
function renderCodexDocument(artifact) {
  ...
  if (artifact.kind === "command") {
    if (artifact.metadata.namespace) {
      headerLines.push(`namespace: ${artifact.metadata.namespace}`);
    }
...
function renderAgentsRuntimeDocument(artifact) {
  ...
  if (artifact.kind === "command") {
    ...
    if (artifact.metadata.namespace) {
      lines.push(`Namespace: ${artifact.metadata.namespace}`);
    }
  }
  lines.push(
    "",
...
function renderCodexCommandIndex(commands) {
  ...
  for (const command of commands) {
    lines.push(
      ...
      "",
      `File: .codex/commands/${command.metadata.commandName}.md`,
    );
  }
...
function renderManagedAgentsBlock(commands) {
  ...
  for (const command of commands) {
    lines.push(
      `- \`${command.metadata.slash}\` -> \`.codex/commands/${command.metadata.commandName}.md\``,
    );
  }
...
function renderManagedInstructionsBlock(targetRoot, sourceRoot) {
  const sourceDisplay = formatPathForDoc(targetRoot, sourceRoot);
  const lines = [
    ...
    "",
    `Canonical source files live in \`${sourceDisplay}\`.`,
...
function stripCommandBlocks(content) {
  return content
    .replace(/<!-- omnibind:commands:start -->[\s\S]*?<!-- omnibind:commands:end -->\n?/g, "")
    .replace(/<!-- skillsync:commands:start -->[\s\S]*?<!-- skillsync:commands:end -->\n?/g, "")
    .trimEnd();
}

function stripInstructionBlocks(content) {
  return content
    .replace(/<!-- skillsync:instructions:start -->[\s\S]*?<!-- skillsync:instructions:end -->\n?/g, "")
    .trimEnd();
}
...
async function syncCodexAgentsFile(projectRoot, commands) {
  ...
  let nextContent;
  const cleanedExisting = existing ? stripCommandBlocks(existing) : existing;
...
async function syncManagedInstructionFile(targetRoot, fileName, sourceRoot) {
  const absolutePath = path.join(targetRoot, fileName);
  const block = renderManagedInstructionsBlock(targetRoot, sourceRoot);
  const existing = await readIfExists(absolutePath);
  const cleanedExisting = existing ? stripInstructionBlocks(existing) : existing;
  const nextContent = !cleanedExisting
    ? `${block}\n`
    : `${cleanedExisting.trimEnd()}\n\n${block}\n`;

  if (existing === nextContent) {
    return { wrote: false, relativePath: fileName };
  }

  await writeAtomic(absolutePath, nextContent);
  return { wrote: true, relativePath: fileName };
}
  ...
  for (const [providerId, providerConfig] of Object.entries(config.providers)) {
    ...
    outputs.push({
      ...
      targetPath: path.join(
        providerConfig.root,
        "commands",
        `${normalizedArtifact.metadata.commandName || normalizedArtifact.name}.md`,
      ),
      content: renderProviderDocument(providerId, normalizedArtifact),
    });
  }
...
async function buildArtifacts(targetRoot, sourceRoot, config, options = {}) {
  const { statePath, data: state } = await loadState(targetRoot);
  const artifacts = await discoverArtifacts(sourceRoot, config);
  const claudePlugin = await loadClaudePluginMetadata(sourceRoot);
  const warnings = [];
  let writes = 0;
  let deletes = 0;
  const nextArtifacts = {};
  ...
  for (const artifact of artifacts) {
    ...
    for (const output of outputs) {
      const absoluteTargetPath = path.join(targetRoot, output.targetPath);
  ...
  const commandArtifacts = artifacts
    ...
    .map((artifact) => ({
      ...
      metadata: {
        description: artifact.entry.data.description || "",
        namespace: artifact.entry.data.namespace || "",
        commandName: artifact.entry.data.namespace
          ? `${artifact.entry.data.namespace}:${artifact.name}`
          : artifact.name,
        slash: artifact.entry.data.slash || `/${artifact.entry.data.namespace ? `${artifact.entry.data.namespace}:${artifact.name}` : artifact.name}`,
      },
    }))
  ...
  if (claudePlugin) {
    for (const artifact of artifacts) {
      ...
      const commandName = `${claudePlugin.name}:${artifact.name}`;
      const commandContent = `---
description: "Invoke the ${claudePlugin.name}:${artifact.name} skill."
disable-model-invocation: true
---

Invoke the ${claudePlugin.name}:${artifact.name} skill and follow it exactly as presented to you.
`;
      const targetPath = path.join(config.providers.claude.root, "commands", `${commandName}.md`);
      const absoluteTargetPath = path.join(targetRoot, targetPath);
      const existingContent = await readIfExists(absoluteTargetPath);
      const existingHash = existingContent ? hashContent(existingContent) : null;
      const nextHash = hashContent(commandContent);
      const previousRecordedHash = state.generatedFiles[targetPath]?.contentHash;
      ...
      if (
        ...
        warnings.push(`Manual edit detected in generated file: ${targetPath}`);
      }

      if (existingHash !== nextHash) {
        await writeAtomic(absoluteTargetPath, commandContent);
        writes += 1;
        if (options.verbose) {
  ...
  if (codexCommandsEnabled) {
    const commandIndexPath = path.join(targetRoot, config.providers.codex.root, "COMMANDS.md");
    ...
    if (existingCommandIndex !== commandIndexContent) {
      ...
      writes += 1;
      if (options.verbose) {
        log(`wrote ${path.relative(targetRoot, commandIndexPath)}`);
      }
    }

    const agentsResult = await syncCodexAgentsFile(targetRoot, commandArtifacts);
  ...
  for (const fileName of ["AGENTS.md", "CLAUDE.md"]) {
    const instructionResult = await syncManagedInstructionFile(targetRoot, fileName, sourceRoot);
    if (instructionResult.wrote) {
      writes += 1;
      if (options.verbose) {
        log(`wrote ${instructionResult.relativePath}`);
      }
    }
  }
  ...
  for (const trackedPath of Object.keys(state.generatedFiles)) {
    ...
    const absoluteTrackedPath = path.join(targetRoot, trackedPath);
    try {
...
async function computeWatchFingerprint(sourceRoot, config) {
  ...
  for (const filePath of files) {
    ...
    descriptors.push({
      file: toPosixPath(path.relative(sourceRoot, filePath)),
      size: stats.size,
...
export async function buildProject(projectRoot, options = {}) {
  ...
  const { config } = await loadConfig(roots.sourceRoot);
  const result = await buildArtifacts(roots.targetRoot, roots.sourceRoot, config, options);
...
export async function watchProject(projectRoot, options = {}) {
  const roots = resolveRoots(projectRoot, options);
  const { config } = await loadConfig(roots.sourceRoot);
  let fingerprint = await computeWatchFingerprint(roots.sourceRoot, config);
  await buildProject(projectRoot, { ...options, verbose: true });
  log("watching for changes");

  setInterval(async () => {
    try {
      const nextFingerprint = await computeWatchFingerprint(roots.sourceRoot, config);
...
export async function doctorProject(projectRoot, options = {}) {
  const roots = resolveRoots(projectRoot, options);
  const { configPath, config } = await loadConfig(roots.sourceRoot);
  const issues = [];

  if (
    !(await fileExists(configPath)) &&
    !(await fileExists(path.join(roots.sourceRoot, "omnibind.config.json")))
  ) {
    issues.push("missing skillsync.config.json");
  }

  for (const [sourceType, relativePath] of Object.entries(config.sources)) {
    if (!(await fileExists(path.join(roots.sourceRoot, relativePath)))) {
