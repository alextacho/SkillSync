...
function printHelp() {
  console.log(`skillsync <command> [--project <path>]
...
export async function runCli(argv) {
  ...
  switch (options.command) {
    case "init":
      ...
      return;
    case "build":
      await buildProject(options.projectRoot, { verbose: true });
      return;
    case "watch":
      await watchProject(options.projectRoot);
      return;
    case "diff":
      await diffProject(options.projectRoot);
      return;
    case "doctor":
      await doctorProject(options.projectRoot);
      return;
    default:
